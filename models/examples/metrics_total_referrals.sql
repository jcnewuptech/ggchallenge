...
select
    c.record_date,
    r.mart_referrals_referral_total_count,
    lag(mart_referrals_referral_total_count) over (
        order by mart_referrals_referral_updated_month asc
    ) as mart_referrals_referral_total_count_last_month
from calendar c
left join
